...
<?php
   ...
   $con= new Dbconn();   
    ...
    $sqlQry = $conn->query("SELECT *  FROM tbl_perordercafe WHERE phone='".$phone."' and status=1 ");
                    ...
                    <div class="card m-b-20">
                                    ...
                                    <div class="col-6" style="text-align: right; ">
                                        <div class="row" >
                                            <div class="col-12">
                                                ...
                                                ?>
                                                <a href="processcafe.php?phonecafe=<?php echo $rows['phone']?>">
                        ...
                        <p class="text-muted"></p>
                            <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                ...
                                <tbody>
                                <?php $sqlQry = $conn->query("SELECT *  FROM tbl_perordercafe WHERE phone='".$phone."' and status=1 ");
                                if($sqlQry->num_rows > 0){
                                    while ($rows = $sqlQry->fetch_array()) {
                                        echo "<tr>";           
                                            ...
                                            echo "<td  style='text-align: center'><img src='upload/cafe/{$rows['image']}' alt='' style=' height: 30px; width: 70px;text-align: center'></td>";
                                            echo "<td style='text-align: center'>{$rows['price']} ៛</td>";      
                                        echo "</tr>";
                                    }
                                }
                                    $sum = $conn->query("SELECT sum(price)+4000 total FROM tbl_perordercafe WHERE phone='".$phone."' and status=1 ");
                                    if($sum->num_rows > 0){
