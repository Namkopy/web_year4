...
<?php
   ...
   $con= new Dbconn();   
    $conn=$con->DbConnect();
   $id=$_GET['id'];
   $sql = "SELECT * FROM tbl_cafe WHERE id=$id";
   ...
   if (isset($_POST['submit'])) {
       ...
       if(!empty($_FILES['img'])){
        ...
        move_uploaded_file($_FILES['img']['tmp_name'],"upload/cafe/".$img);
       
     }
        if($img==''){

            $img=$result['image'];            
            $sqlupdate="UPDATE tbl_cafe set name='".$name."',size='".$size."',type='".$type."',detail='".$des."',image='".$img."',price='".$price."' WHERE id=$id";
        ...
        if(!$sqlQry1){
            ...
            echo "<script> location.href='cafe.php'; </script>";
            // $print_r($_FILES);
        }
                            ...
                            <form action="#" method="post" enctype="multipart/form-data">
                                <div class="form-group"><input type="text" class="form-control"
                                        required placeholder="Name Of Drink" name="name"
                                         value="<?php echo $result['name'] ?>"></div>
                                <div class="form-group">
                                <div class="form-group row" id="type">
                                    <div class="col-sm-12">
                                        <select class="form-control"  name="size" value="<?php echo $result['name'] ?>" >
                                            <?php 
                                            if($result['size']=='L'){
                                    ...
                                    <div class="col-sm-12">
                                        <select class="form-control"  name="type">
                                            <?php 
                                                ...
                                                else if($result['type']=='Shake'){
                                                    echo "<option value='Shake'>Shake</option>
                                                    <option value='Hot'>Hot</option>
