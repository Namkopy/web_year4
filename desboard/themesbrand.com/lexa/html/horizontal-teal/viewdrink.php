...
<?php
   ...
   $con= new Dbconn();   
    ...
    $sqlQry = $conn->query("SELECT *  FROM tbl_perorderdrink WHERE phone='".$phone."' and status=1 ");
                            ...
                            <div class="container">
                                <div class="row">
                                    ...
                                    <div class="col-6" style="text-align: right; ">
                                        <div class="row" >
                                        <div class="col-12">
                                            <?php   $sqlQry = $conn->query("SELECT *  FROM tbl_perorderdrink WHERE phone='".$phone."' and status=1 ");
                                            if($sqlQry->num_rows > 0){
                                                $rows = $sqlQry->fetch_array();} ?>
                                                 <a href="processdrink.php?phone=<?php echo $rows['phone']?>">
                                ...
                                if($sqlQry->num_rows > 0){
                                    while ($rows = $sqlQry->fetch_array()) {
                                        echo "<tr>";           
                                            ...
                                            echo "<td  style='text-align: center'><img src='upload/drink/{$rows['image']}' alt='' style=' height: 30px; width: 70px;text-align: center'></td>";
                                            echo "<td style='text-align: center'>{$rows['price']} ៛</td>";      
                                        echo "</tr>";
                                    }
                                }
                                    $sum = $conn->query("SELECT sum(price)+4000 total FROM tbl_perorderdrink WHERE phone='".$phone."' and status=1 ");
                                    if($sum->num_rows > 0){
                                       $rows = $sum->fetch_array();}

                                    echo "<tr style='text-align: right'>
                                        <td colspan='3' style='text-align: right;color:#44bd32;font-weight: bold' >   Total :   </td>
                                        <td colspan='4' style='text-align: center;color:red'>  {$rows['total']} ៛ </td>
